...
- **ComfyUI Pinned to `v0.34.2`**: The base image bakes ComfyUI `0.30.0`, which is below the `0.34.0` floor the latest workflows need, so the build checks out `v0.34.2` explicitly. See below.
...
`v0.34.2` is the newest tag on origin. When moving to a newer ComfyUI,
...
docker buildx build \
  ...
  --build-arg COMFYUI_MIN_VERSION=0.34.0 \
  --build-arg COMFYUI_REF=v0.34.2 \
